...
class ApiHttpClient {
  ...
  Future<http.Response> multipartPost(
    ...
    // Nota: la logica di refresh token per multipart è complessa (bisogna ricreare lo stream dei file).
    // Per ora, se il token è scaduto durante un upload, fallirà e l'utente dovrà riprovare (o fare logout/login).
    if (authenticated && _isAuthError(response.statusCode)) {
      // Tentativo semplificato: se refresh ha successo, l'utente è loggato, ma ritorniamo comunque l'errore 401
      // per questa richiesta specifica, lasciando che sia la UI o un retry manuale a gestire la cosa.
