...
const { User, RefreshToken, AuditLog, Vehicle, Spot, FavoriteSpot, MaintenanceEntry } = require('../models');
...
// Limite per endpoint sensibili (es. reset password)
...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    // Assicura username univoco: append counter se già esiste
    ...
    user = await User.create({ username: uniqueUsername, email, password });
    // Genera token di verifica
    const verificationToken = jwt.sign({ id: user.id }, process.env.JWT_SECRET || 'testsecret', { expiresIn: '1d' });
    // In produzione invia l'email di verifica, altrimenti salta
    if (process.env.NODE_ENV === 'production') {
      try {
        await sendVerificationEmail(user, verificationToken);
      } catch (err) {
        console.warn('Errore invio email verifica:', err.message);
      }
    } else {
      console.log('Saltata invio email verifica in ambiente non-production');
    }
...
// SOLO DEV: forza emailVerified=true per un utente (debug)
router.post('/dev-verify-email', async (req, res) => {
  ...
  try {
    ...
    user.emailVerified = true;
    await user.save();
    return res.json({ message: 'Email verificata forzata (dev)', email: user.email });
  } catch (err) {
    console.error('Errore dev-verify-email:', err);
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    // Genera e persiste token + audit log
    const { token, refreshToken } = await handleLogin(user, req);
...
router.delete('/account', async (req, res) => {
  ...
  try {
    ...
    // Cancella tutti i veicoli dell'utente
    await Vehicle.destroy({ where: { userId } });
    // Cancella tutti gli spot creati dall'utente
    await Spot.destroy({ where: { userId } });
    // Cancella tutti i preferiti dell'utente
    await FavoriteSpot.destroy({ where: { userId } });
    // Cancella tutte le maintenance entries dell'utente
    await MaintenanceEntry.destroy({ where: { userId } });
    // Anonimizza i log dell'utente (mantieni la traccia ma rimuovi dati personali)
    await AuditLog.update(
      { deviceInfo: null, ip: null },
      { where: { userId } }
    );
    // Cancella l'utente
    const deleted = await User.destroy({ where: { id: userId } });
    if (deleted) {
      // Audit log: cancellazione account (anonimo)
      await AuditLog.create({
        userId: null,
...
// Esportazione dati personali utente (GDPR)
router.get('/export-data', async (req, res) => {
  ...
  try {
    ...
    // Spot preferiti
    const favoriteSpots = await FavoriteSpot.findAll({ where: { userId } });
    // Manutenzioni
    const maintenanceEntries = await MaintenanceEntry.findAll({ where: { userId } });
    res.json({
      user,
      vehicles,
      spots,
